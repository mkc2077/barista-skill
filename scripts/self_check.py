...
1. 29 MCP tools: SKILL.md declared names == server.py actual @mcp.tool names
...
def run_checks() -> int:
    ...
    lst = re.search(r"`add_knowledge.*?search_sca_sources`", skill_text, re.DOTALL)
    if not lst:
        fail("SKILL.md tool list not found", "looking for `add_knowledge ... search_sca_sources` block")
        fails += 1
        declared = set()
    else:
        declared = {n for n in re.findall(r"\b(add_\w+|get_\w+|diagnose_\w+|calculate_\w+|calibrate_\w+|search_\w+|identify_\w+|start_\w+|log_\w+|next_\w+|rag_\w+|sync_\w+|check_\w+|set_\w+)\b", lst.group(0))}

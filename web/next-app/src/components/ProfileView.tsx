...
export function ProfileView() {
  ...
  return (
    <div className='flex-1 min-h-0 overflow-y-auto'>
      {/* ── Hero 头部：CONACH 深底海报风 — 模块 hue 浸染 + 大写 tracked 标题 ── */}
      <header
        className='hero-bath relative px-6 py-10 overflow-hidden'
      >
        <div className='hero-cloud' aria-hidden='true' />
        <div
          className='absolute right-8 top-1/2 -translate-y-1/2 opacity-30 pointer-events-none hidden md:block'
          style={{ color: 'var(--accent)' }}
        >
          <Coffee className='w-20 h-20' strokeWidth={1.2} />
        </div>
        <div className='hero-content relative max-w-2xl mx-auto flex items-start justify-between gap-4'>
          <div>
            <div className='font-hero-eyebrow mb-3'>
              Profile & Gear · 我的资料
            </div>
            <h1 className='font-hero-display'>
              {profileComplete > 0 ? 'Profile Filled' : 'Welcome'}
            </h1>
            <p className='font-hero-cn mt-2 max-w-md'>
              {profileComplete > 0
                ? `画像完成度 ${profileComplete}/4 · ${devices.grinders.length + devices.brewers.length} 台设备 · ${moduleTasteCount}/6 模块口味 · 材料 ${items.length} 件`
                : '记录你的设备、口味和手头材料，让顾问每次对话都精准推荐。'}
            </p>
          </div>
          <button
            onClick={() => setViewMode('chat')}
            className='hero-back shrink-0'
          >
                    ...
                    return (
                      <button
                        ...
                        className={
                          ...
                          (isSelected
                            ? 'chip-conach active text-[var(--text-secondary)] font-medium'
                            : 'chip-conach hover:border-[var(--accent)] hover:text-[var(--text-secondary)]')
                        }
                        title={t.label}
                      >
                        {t.short}
              ...
              return (
                <button
                  ...
                  className={
                    ...
                    (isActive
                      ? 'chip-conach active text-[var(--text-secondary)]'
                      : 'chip-conach hover:border-[var(--accent)] hover:text-[var(--text-secondary)]')
                  }
                >
      ...
      <div className='relative'>
        <div className='flex items-center justify-between mb-1'>
          <div className='flex items-center gap-2'>
            ...
            <h2 className='font-editorial text-lg text-[var(--text-secondary)]'>{title}</h2>
          </div>
    ...
    <div className='mb-3'>
      ...
      {items.length > 0 && (
        <div className='flex flex-wrap gap-1.5 mb-2'>
          {items.map((it) => (
            <span key={it} className='inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-md border chip-conach active text-[var(--text-secondary)]'>
              {it}
